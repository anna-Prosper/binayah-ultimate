...
export function MarketReports({ t }: { t: T }) {
  return (
    <Browser t={t} url="reports/apr-2026">
      ...
      <div style={{background:t.surface,borderRadius:8,padding:"4px 4px",marginTop:4}}>
    ...
    <Browser t={t} url="video/creek-vista-t3">
      ...
      <div style={{display:"flex",gap:0,justifyContent:"center"}}>
        {["🇬🇧 EN","🇷🇺 RU","🇨🇳 ZH","🇹🇷 TR","🇮🇳 HI","🇩🇪 DE"].map(x=>(
          <span key={x} style={{fontSize:5.5,background:t.surface,padding:"0 4px",borderRadius:3,color:t.cyan||t.accent}}>{x}</span>
        ))}
      </div>
    </Browser>
  );
...
export function MarketDashboard({ t }: { t: T }) {
  return (
    <Browser t={t} url="binayah.com/market">
      ...
      <div style={{fontSize:5.5,color:t.textDim,marginTop:4}}>🤖 AI: Marina +9% projected next 12mo</div>
    </Browser>
  );
}
