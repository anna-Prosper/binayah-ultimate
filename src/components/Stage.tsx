      ...
      <div onClick={handleCardClick} style={{ flex: 1, background: isE ? t.bgHover : t.bgSoft, border: `1px solid ${hasLive ? liveColor + "66" : isE ? pC + "33" : t.border}`, borderRadius: 16, marginBottom: idx < tot - 1 ? 6 : 0, cursor: "pointer", transition: "border-color 0.4s, box-shadow 0.4s, background 0.2s", overflow: "hidden", boxShadow: hasLive ? `${isE ? t.shadowLg : t.shadow}, 0 0 16px ${liveColor}22` : isE ? t.shadowLg : t.shadow }}>
        ...
        <div style={{ padding: isMobile ? "10px 12px 4px" : "10px 14px 4px", display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "flex-start" : "center", gap: isMobile ? 4 : 8 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4, minWidth: 0, flex: 1, width: "100%" }}>
            ...
            <span onClick={e => { e.stopPropagation(); cycleStatus(name); }} style={{ fontSize: 7, fontWeight: 700, color: st.c, background: st.c + "12", padding: "0 8px", borderRadius: 8, flexShrink: 0, cursor: isLocked ? "not-allowed" : "pointer", opacity: isLocked ? 0.6 : 1 }} title={isLocked ? "Pipeline is locked" : "Click to cycle status"}>{st.l}</span>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 0, position: "relative", flexWrap: "wrap", ...(isMobile ? { paddingLeft: 20 } : {}) }} onClick={e => e.stopPropagation()}>
            ...
            {/* React toggle */}
            {!isLocked && <button onClick={() => setReactOpen(reactOpen === name ? null : name)} style={{ background: "transparent", border: `1px solid ${t.border}`, borderRadius: 8, padding: isMobile ? "8px 12px" : "4px 8px", minHeight: isMobile ? 44 : undefined, cursor: "pointer", fontSize: 9, color: t.textMuted, fontFamily: "inherit" }}>
              {"\uD83D\uDE00"}
            </button>}

            {/* Preview badge — indicates stage has a mockup */}
            {MockupComp && !(isMobile ? mobileSheetOpen : isE) && (
              <span style={{ fontSize: 7, color: pC, background: pC + "15", border: `1px solid ${pC}22`, borderRadius: 8, padding: "0 4px", fontFamily: "var(--font-dm-mono), monospace", fontWeight: 700, flexShrink: 0, opacity: 0.8 }}>▸</span>
            )}
        ...
        <div style={{ paddingLeft: 32, paddingRight: 12, paddingBottom: 8 }} onClick={e => e.stopPropagation()}>
          {editingShortDesc && !isLocked ? (
            <input
              ...
              disabled={isLocked}
              style={{ width: "100%", background: "transparent", border: "none", borderBottom: `1px solid ${pC}44`, outline: "none", fontSize: 9, color: t.textSec, fontFamily: "var(--font-dm-sans), sans-serif", padding: "0 0 0 0", lineHeight: 1.4 }}
            />
          ) : (
            <p
        ...
        {isE && (
          <div style={{ borderTop: `1px solid ${isLocked ? t.amber + "33" : t.border}`, animation: "fadeIn 0.2s ease", boxShadow: isLocked ? `inset 0 0 0 1px ${t.amber}22` : "none" }} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                {!claimedBy.includes(currentUser!) ? (
                  <button onClick={() => handleClaim(name)} style={{ background: `linear-gradient(135deg,${me?.color || t.accent},${me?.color || t.accent}aa)`, border: "none", borderRadius: 12, padding: "8px 20px", cursor: "pointer", fontSize: 11, color: "#fff", fontWeight: 800, fontFamily: "var(--font-dm-mono), monospace", textTransform: "lowercase", boxShadow: `0 0 20px ${me?.color || t.accent}44, 0 2px 8px rgba(0,0,0,0.4)`, display: "flex", alignItems: "center", gap: 8, animation: "claimPulse 2s ease-in-out infinite", position: "relative", overflow: "hidden", letterSpacing: 0.3, width: isMobile ? "100%" : undefined, justifyContent: isMobile ? "center" : undefined, minHeight: 44 }}>
                    ...
                    <span style={{ background: "rgba(255,255,255,0.15)", borderRadius: 8, padding: "0 8px", fontSize: 8 }}>earn +{s.points} on live</span>
                ...
                }} style={{ background: "transparent", border: `1px solid ${t.border}`, borderRadius: 12, padding: "4px 12px", cursor: "pointer", fontSize: 9, color: copied === name ? t.green : t.textMuted, fontWeight: 600, fontFamily: "var(--font-dm-mono), monospace", transition: "all 0.15s" }}>
            ...
            <div style={{ padding: "8px 16px", borderBottom: `1px solid ${t.border}` }}>
              <div style={{ fontSize: 7, color: t.textDim, letterSpacing: 2, textTransform: "uppercase", marginBottom: 4, fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}>
                about
              ...
              {editingDesc && !isLocked ? (
                <textarea
                  ...
                  disabled={isLocked}
                  rows={4}
                  style={{ width: "100%", background: t.bgHover, border: `1px solid ${pC}44`, borderRadius: 8, padding: "4px 8px", fontSize: 11, color: t.textSec, fontFamily: "var(--font-dm-sans), sans-serif", outline: "none", resize: "none", lineHeight: 1.6 }}
                />
              ) : (
                <div onClick={() => { if (!isLocked) setEditingDesc(true); }} title={isLocked ? "Pipeline is locked" : "Click to edit"} style={{ fontSize: 11, color: t.textSec, lineHeight: 1.6, cursor: isLocked ? "not-allowed" : "text", display: "flex", alignItems: "flex-start", gap: 4 }}>
            ...
            <div style={{ display: "flex", gap: 0, minHeight: 80 }}>
              <div style={{ flex: 1, padding: "12px 16px", borderRight: `1px solid ${t.border}`, pointerEvents: isLocked ? "none" : "auto" }}>
                ...
                {tasks.map(task => (
                  <div key={task.id} style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 0", borderRadius: 8, transition: "background 0.15s" }}
                    ...
                    <span style={{ fontSize: 7, color: t.textDim, marginRight: 0 }}>{users.find(u => u.id === task.by)?.name?.charAt(0)}</span>
                ...
                <div style={{ display: "flex", gap: 4, marginTop: 8 }}>
                  <input value={subtaskInput[name] || ""} onChange={e => { if (!isLocked) setSubtaskInput(prev => ({ ...prev, [name]: e.target.value })); }} onKeyDown={e => { if (e.key === "Enter") addSubtask(name); }} placeholder={isLocked ? "pipeline is locked" : "+ add subtask..."} disabled={isLocked} style={{ flex: 1, background: "transparent", border: `1px solid ${isLocked ? t.amber + "22" : t.border}`, borderRadius: 8, padding: "4px 8px", fontSize: 9, color: t.text, fontFamily: "inherit", outline: "none", cursor: isLocked ? "not-allowed" : "text" }} />
                  <button onClick={() => addSubtask(name)} disabled={isLocked} style={{ background: isLocked ? t.surface : t.accent + "15", border: `1px solid ${isLocked ? t.border : t.accent + "33"}`, borderRadius: 8, padding: "4px 8px", cursor: isLocked ? "not-allowed" : "pointer", fontSize: 9, color: isLocked ? t.textDim : t.accent, fontWeight: 700, fontFamily: "inherit" }}>+</button>
                </div>
              </div>
              ...
              <div style={{ flex: 1, padding: "12px 16px", borderRadius: "0 0 14px 0", transition: "box-shadow 0.3s", animation: liveNotifs[name]?.comment ? "commentPulse 1.5s ease-in-out 2" : "none" }}>
                <div style={{ fontSize: 8, color: liveNotifs[name]?.comment ? t.green : t.textDim, letterSpacing: 2, textTransform: "uppercase", marginBottom: 8, fontWeight: 600, transition: "color 0.4s", display: "flex", alignItems: "center", gap: 4 }}>
                ...
                <div style={{ display: "flex", gap: 4, marginTop: 8 }}>
                  ...
                  <button onClick={() => addComment(name)} disabled={isLocked} style={{ background: isLocked ? t.surface : t.accent + "15", border: `1px solid ${isLocked ? t.border : t.accent + "33"}`, borderRadius: 8, padding: "4px 8px", cursor: isLocked ? "not-allowed" : "pointer", fontSize: 9, color: isLocked ? t.textDim : t.accent, fontWeight: 700, fontFamily: "inherit" }}>{"\u21B5"}</button>
                </div>
              </div>
            </div>
        ...
        >
          <div style={{ padding: "0 0 env(safe-area-inset-bottom, 0)" }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                {!claimedBy.includes(currentUser!) ? (
                  <button onClick={() => handleClaim(name)} style={{ background: `linear-gradient(135deg,${me?.color || t.accent},${me?.color || t.accent}aa)`, border: "none", borderRadius: 12, padding: "12px 20px", cursor: "pointer", fontSize: 12, color: "#fff", fontWeight: 800, fontFamily: "var(--font-dm-mono), monospace", textTransform: "lowercase", width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, minHeight: 44 }}>
                    ...
                    <span>claim this</span>
                    <span style={{ background: "rgba(255,255,255,0.15)", borderRadius: 8, padding: "0 8px", fontSize: 9 }}>earn +{s.points} on live</span>
                  </button>
                ) : (
                  <button onClick={() => handleClaim(name)} title="Click to unclaim" style={{ background: t.green + "15", border: `1px solid ${t.green}44`, borderRadius: 12, padding: "12px 16px", cursor: "pointer", fontSize: 12, color: t.green, fontWeight: 800, fontFamily: "var(--font-dm-mono), monospace", textTransform: "lowercase", display: "flex", alignItems: "center", justifyContent: "center", gap: 4, width: "100%", minHeight: 44 }}>
            ...
            <div style={{ padding: "12px 16px", borderBottom: `1px solid ${t.border}` }}>
              <div style={{ fontSize: 7, color: t.textDim, letterSpacing: 2, textTransform: "uppercase", marginBottom: 4, fontWeight: 600 }}>about</div>
                ...
                {cmts.map(c => { const u = users.find(x => x.id === c.by); return (
                  <div key={c.id} style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 12, color: t.textSec, lineHeight: 1.5, marginTop: 0 }}>{c.text}</div>
                    </div>
                  </div>
                ); })}
              </div>
            ...
            {MockupComp && (
              <div style={{ padding: "0 16px 16px", borderTop: `1px solid ${t.border}` }}>
                <div style={{ fontSize: 7, color: pC, letterSpacing: 1.5, textTransform: "uppercase", fontWeight: 700, padding: "8px 0 8px", opacity: 0.8 }}>▸ live preview</div>
