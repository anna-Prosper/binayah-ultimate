    ...
    <section style={{ marginBottom: 18, background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 14, padding: 16 }}>
      <button type="button" onClick={toggleSection} style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10, width: "100%", background: "transparent", border: "none", padding: 0, cursor: "pointer", marginBottom: (!sectionCollapsed && (open || relevant.length > 0)) ? 12 : 0 }}>
        <div style={{ textAlign: "left" as const }}>
          <div style={{ fontSize: 11, color: t.accent, fontFamily: mono, fontWeight: 800, letterSpacing: 0.6, textTransform: "uppercase" as const, display: "flex", alignItems: "center", gap: 6 }}>
            reminders
            {relevant.length > 0 && <span style={{ background: t.amber + "22", border: `1px solid ${t.amber}44`, color: t.amber, borderRadius: 8, padding: "0 5px", fontSize: 10, fontFamily: mono, fontWeight: 800 }}>{relevant.length}</span>}
          </div>
          <div style={{ marginTop: 3, fontSize: 16, color: t.text, fontWeight: 900 }}>dated app + email notifications</div>
        </div>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          <span style={{ fontSize: 12, color: t.textDim, fontFamily: mono }}>{sectionCollapsed ? "▼" : "▲"}</span>
        </div>
      </button>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "minmax(160px, .8fr) minmax(180px, 1fr) auto", gap: 8, alignItems: "start", marginBottom: 12 }}>
          ...
          <div style={{ gridColumn: "1 / -1", display: "flex", flexWrap: "wrap", gap: 5 }}>
            {users.filter(u => u.id !== "ai").map(u => {
              ...
              return (
                <button key={u.id} type="button" onClick={() => toggleRecipient(u.id)} style={{ background: selected ? u.color + "20" : t.bgHover || t.bgSoft, border: `1px solid ${selected ? u.color + "77" : t.border}`, color: selected ? u.color : t.textMuted, borderRadius: 999, padding: "4px 9px", fontSize: 12, fontFamily: mono, fontWeight: selected ? 800 : 600, cursor: "pointer" }}>
                  {selected ? "✓ " : ""}{u.name}
                </button>
              );
            })}
            <button type="button" onClick={submit} disabled={!title.trim() || !remindAt || recipientIds.length === 0} style={{ marginLeft: "auto", background: title.trim() && remindAt && recipientIds.length ? t.green + "22" : t.bgHover || t.bgSoft, border: `1px solid ${title.trim() && remindAt && recipientIds.length ? t.green + "55" : t.border}`, color: title.trim() && remindAt && recipientIds.length ? t.green : t.textDim, borderRadius: 8, padding: "4px 11px", fontSize: 12, fontFamily: mono, fontWeight: 800, cursor: title.trim() && remindAt && recipientIds.length ? "pointer" : "not-allowed" }}>schedule</button>
          </div>
        </div>
      )}
            ...
            return (
              <div key={r.id} style={{ display: "flex", gap: 8, alignItems: "center", border: `1px solid ${due ? t.amber + "55" : t.border}`, background: due ? t.amber + "0d" : t.bgHover || t.bgSoft, borderRadius: 9, padding: "7px 9px" }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, color: t.text, fontWeight: 800, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.title}</div>
                  <div style={{ fontSize: 11, color: due ? t.amber : t.textMuted, fontFamily: mono }}>{due ? "due now" : new Date(r.remindAt).toLocaleString()} · {r.body || "no note"}</div>
                </div>
                {due && <button type="button" onClick={() => onDismiss(r.id)} style={{ background: "transparent", border: `1px solid ${t.border}`, color: t.textDim, borderRadius: 7, padding: "3px 8px", fontSize: 11, fontFamily: mono, fontWeight: 800, cursor: "pointer" }}>done</button>}
              </div>
            );
          })}
        </div>
