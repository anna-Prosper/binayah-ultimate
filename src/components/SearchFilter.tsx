  ...
  return (
    <div style={{ display: "flex", gap: 4, marginBottom: 8, alignItems: "center", flexWrap: "wrap" }}>
      <div style={{ flex: "1 1 200px", position: "relative" }}>
        ...
        <span style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", fontSize: 11, color: t.textDim, fontFamily: "var(--font-dm-mono), monospace", background: t.surface, border: `1px solid ${t.border}`, borderRadius: 8, padding: "0 4px", pointerEvents: "none", whiteSpace: "nowrap" }}>⌘K</span>
      </div>
      <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
        {[{ l: "all", v: null }, { l: "live", v: "active" }, { l: "building", v: "in-progress" }, { l: "planned", v: "planned" }, { l: "concept", v: "concept" }, { l: "my claims", v: "claimed" }].map(f => (
          <button key={f.l} onClick={() => setStatusFilter(statusFilter === f.v ? null : f.v)} style={{ background: statusFilter === f.v ? t.accent + "20" : t.bgCard, border: `1px solid ${statusFilter === f.v ? t.accent + "55" : t.border}`, borderRadius: 8, padding: "4px 8px", fontSize: 11, color: statusFilter === f.v ? t.accent : t.textMuted, fontWeight: statusFilter === f.v ? 700 : 500, cursor: "pointer", fontFamily: "var(--font-dm-mono), monospace", transition: "all 0.15s" }}>{f.l}</button>
        ))}
      </div>
    </div>
  );
