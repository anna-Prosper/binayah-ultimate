  ...
  if (onboardStep === 6) {
    ...
    return (
      <div style={{ position: "fixed", inset: 0, background: t.bg, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, fontFamily: "var(--font-dm-sans), sans-serif" }}>
        <style>{css}</style>
        <AnimBg />
        <NB color={user.color} style={{ background: t.bgCard, padding: "32px 28px", maxWidth: 420, width: "92%", textAlign: "center", animation: "scaleIn 0.4s ease", position: "relative", zIndex: 1 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, maxWidth: 288, margin: "0 auto 28px" }}>
            {AVATARS.map((av, idx) => {
              const active = selAvatar === av.id;
              return (
                <button key={av.id} onClick={() => setSelAvatar(av.id)} style={{
                  width: 56, height: 56, borderRadius: 16,
                  background: active ? `radial-gradient(circle, ${user.color}33, ${user.color}11)` : "transparent",
                  border: `2px solid ${active ? user.color : t.border}`,
                  display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
                  fontSize: 24, cursor: "pointer", transition: "all 0.2s",
                  boxShadow: active ? `0 0 20px ${user.color}33` : "none",
                  transform: active ? "scale(1.1)" : "scale(1)",
                  animation: `scaleIn 0.3s ease ${idx * 0.02}s both`,
                }}>
                  {av.emoji}
                  <span style={{ fontSize: 6, color: active ? user.color : t.textDim, marginTop: 1 }}>{av.name}</span>
                </button>
              );
            })}
          </div>
          ...
          <button onClick={() => {
            const updated = users.map(u => u.id === selUser ? { ...u, avatar: selAvatar || u.avatar } : u);
            setUsers(updated); setCurrentUser(selUser); setTimeout(() => setOnboardStep(7), 50);
          }} style={{
