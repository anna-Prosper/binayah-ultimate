...
export default function KanbanView({ t, getStatus, setStageStatusDirect, claims, reactions, users, currentUser, sc, ck, customStages, customPipelines, onCardClick, searchQ }: Props) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 24, animation: "fadeIn 0.2s ease" }}>
      {COLS.map(col => {
        ...
        return (
          <div
            ...
            style={{ background: dragOver === col.id ? stc.c + "0c" : t.bgSoft, border: `2px solid ${dragOver === col.id ? stc.c + "55" : t.border}`, borderRadius: 16, padding: "12px 10px", minHeight: 420, transition: "all 0.15s" }}
          >
            {/* Column header */}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {stages.map(s => {
                ...
                return (
                  <div
                    ...
                    style={{ background: t.bgCard, border: `1px solid ${dragging === s.name ? stc.c + "66" : t.border}`, borderLeft: `3px solid ${s.pipelineColor}`, borderRadius: 12, padding: "10px 12px", cursor: "grab", opacity: dragging === s.name ? 0.4 : 1, transition: "all 0.15s", boxShadow: t.shadow, userSelect: "none" }}
                  >
                    {/* Pipeline tag */}
                    <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 6 }}>
                      <span style={{ fontSize: 9 }}>{s.pipelineIcon}</span>
                      <span style={{ fontSize: 7, color: s.pipelineColor, fontFamily: "var(--font-dm-mono), monospace", fontWeight: 700, background: s.pipelineColor + "15", padding: "1px 6px", borderRadius: 5, maxWidth: 110, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.pipelineName}</span>
                    </div>
                    {/* Name */}
                    <div style={{ fontSize: 11, fontWeight: 700, color: t.text, marginBottom: 4, lineHeight: 1.3 }}>{s.name}</div>
                    {/* Desc */}
                    {def.desc && <div style={{ fontSize: 8, color: t.textSec, lineHeight: 1.4, marginBottom: 7, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as const }}>{def.desc}</div>}
                    {/* Footer */}
                    <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 4 }}>
                      {claimedBy.length > 0
                        ? <div style={{ display: "flex" }}>{claimedBy.slice(0, 3).map(uid => { const u = users.find(u => u.id === uid); return u ? <div key={uid} style={{ marginLeft: -3 }}><AvatarC user={u} size={16} /></div> : null; })}</div>
              ...
              {stages.length === 0 && (
                <div style={{ textAlign: "center", padding: "40px 8px", color: t.textDim, fontSize: 8, fontFamily: "var(--font-dm-mono), monospace", borderRadius: 10, border: `2px dashed ${t.border}` }}>
                  drag here to move
                </div>
              )}
            </div>
