  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", gap: 10, alignItems: fullPage ? "flex-end" : "center", justifyContent: "space-between", marginBottom: 8, flexWrap: "wrap" }}>
        <div>
          <div style={{ fontSize: 10, color: t.accent, fontFamily: mono, fontWeight: 900, textTransform: "uppercase", letterSpacing: 0.7 }}>archive</div>
        ...
        {fullPage && <input value={query} onChange={e => setQuery(e.target.value)} placeholder="search archive..." style={{ width: 260, maxWidth: "100%", background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 10, padding: "9px 11px", color: t.text, outline: "none", fontFamily: mono, fontSize: 12 }} />}
      </div>
      {nothingArchived ? (
        <div style={{ fontSize: 12, color: t.textDim, fontFamily: mono, lineHeight: 1.6 }}>
          // nothing archived in shared state right now.
          <br />Archive is a restore list, not a timed trash bin.
        </div>
      ) : nothingFiltered ? (
        <div style={{ fontSize: 11, color: t.textDim, fontFamily: mono, border: `1px dashed ${t.border}`, borderRadius: 12, padding: 24 }}>// no archived items match this search</div>
      ) : (
        <>
          {filteredStages.length > 0 && (
            <div>
              <div style={{ fontSize: 10, color: t.textMuted, fontWeight: 700, letterSpacing: 0.5, textTransform: "uppercase", fontFamily: mono, marginBottom: 6 }}>tasks ({filteredStages.length})</div>
              {filteredStages.map(sid => (
                <div key={sid} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 8px", borderRadius: 8, background: t.bgHover, marginBottom: 4 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, color: t.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{stageNameOverrides[sid] || sid}</div>
                    {archiveMeta.get(sid) && <div style={{ fontSize: 10, color: t.textDim, fontFamily: mono }}>{archiveMeta.get(sid)}</div>}
                  </div>
          ...
          {filteredSubtasks.length > 0 && (
            <div>
              <div style={{ fontSize: 10, color: t.textMuted, fontWeight: 700, letterSpacing: 0.5, textTransform: "uppercase", fontFamily: mono, marginBottom: 6 }}>subtasks ({filteredSubtasks.length})</div>
              {filteredSubtasks.map(s => (
                <div key={s.key} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 8px", borderRadius: 8, background: t.bgHover, marginBottom: 4 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: t.textDim, fontFamily: mono, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>under: {s.parentStageName}{archiveMeta.get(s.key) ? ` · ${archiveMeta.get(s.key)}` : ""}</div>
                  </div>
          ...
          {filteredPipelines.length > 0 && (
            <div>
              <div style={{ fontSize: 10, color: t.textMuted, fontWeight: 700, letterSpacing: 0.5, textTransform: "uppercase", fontFamily: mono, marginBottom: 6 }}>pipelines ({filteredPipelines.length})</div>
