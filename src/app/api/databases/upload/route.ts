...
// POST /api/databases/upload — upload a file/image and return attachment metadata
// for a database row. The row itself lives in the shared state and persists via
// the standard pipeline-state patch flow.
export async function POST(req: NextRequest) {
  const session = await getServerSession(authOptions);
  if (!session) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const rl = rateLimit(req, "/api/databases/upload", 20, 60_000);
  if (!rl.ok) {
  ...
  try {
    ...
    uploaded = await uploadToS3("databases", file.name, buf, contentType);
  } catch (e) {
    console.error("[databases/upload] upload failed:", e);
